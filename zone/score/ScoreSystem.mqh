...
CArrayObj* findZoneScores(CArrayObj* zones){
      CArrayObj* zoneDataList = sort(zones);
      return zoneDataList;
}

CArrayObj* filterOutWeakScores(CArrayObj* zoneDataList){
...
double score(Zone* obj, int touchIndex){
    ...
    //broken points
    BreakDetectionParams *params = new BreakDetectionParams(0, store.THRESHOLD_BROKEN_LEVEL);
